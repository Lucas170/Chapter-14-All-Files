...
/*----- AIM -----
   ...
   Replicate the SMA indicator on closing prices without using the iMA function
...
//----
   ...
   for(i=limit; i>=0; i--) { // This is where we calculate our Indicator value and place them in our Buffer(s)
         ...
         total = 0;
         for(x=i; x<i+period; x++) {
            total = total + iClose(NULL, 0, x);
         }
         
         // TDL 2: Divide the Sum by X. Then assign the values to the buffer. 
         ExtMapBuffer1[i]=total/period; // Sum of prices divided number of bars gives us the SMA on closing price 
